...
abstract contract AuthModule is AuthInternals, GovernanceModule {
        ...
        for (uint256 i; i < length_; ) {
            ...
            uint256 revenueAmount_ = LiquidityCalcs.calcRevenue(
                ...
                (isNativeToken_ ? address(this).balance : IERC20(tokens_[i]).balanceOf(address(this))) +
                    _getExternalBalances(tokens_[i])
            );
            ...
            if (revenueAmount_ > 0) {
                ...
                if (isNativeToken_) {
                    ...
                    _preTransferOut(tokens_[i], revenueAmount_);
    ...
    function updateTokenConfigs(TokenConfig[] calldata tokenConfigs_) external onlyAuths {
        ...
        for (uint256 i; i < length_; ) {
            ...
            if (supplyExchangePrice_ > 0 && borrowExchangePrice_ > 0) {
                // update the current exchange prices based on earned interest before updating fee + timestamp in storage
            ...
            // trigger update borrow rate, utilization, ratios etc. as the updated fee affects these values.
            _updateExchangePricesAndRates(tokenConfigs_[i].token);
    ...
    function updateUserWithdrawalLimit(address user_, address token_, uint256 newLimit_) external onlyAuths {
        ...
        _userSupplyData[user_][token_] =
            // mask to update bits 65-161 (withdrawal limit, timestamp) and reset decay bits 218-253
            (userSupplyData_ & 0xC000000003FFFFFFFFFFFFFC000000000000000000000001FFFFFFFFFFFFFFFF) |
    ...
    function updateUserSupplyConfigs(UserSupplyConfig[] memory userSupplyConfigs_) external onlyAuths {
        ...
        for (uint256 i; i < userSupplyConfigs_.length; ) {
            ...
            if (
                ...
                // mask to update first bit + bits 162-217 (expand percentage, expand duration, base limit) and reset decay bits 218-253
                _userSupplyData[userSupplyConfigs_[i].user][userSupplyConfigs_[i].token] =
                    (userSupplyData_ & 0xC00000000000000000000003FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFE) |
                ...
                (supplyExchangePrice_, ) = _updateExchangePrices(userSupplyConfigs_[i].token);
                ...
                // mask to update bits 0-128, bits 162-217 (all except last process timestamp) and reset decay bits 218-253
                _userSupplyData[userSupplyConfigs_[i].user][userSupplyConfigs_[i].token] =
                    (userSupplyData_ & 0xC00000000000000000000003FFFFFFFE00000000000000000000000000000000) |
    ...
    function updateUserBorrowConfigs(UserBorrowConfig[] memory userBorrowConfigs_) external onlyAuths {
        ...
        for (uint256 i; i < userBorrowConfigs_.length; ) {
            ...
            if (
                ...
                // if the mode changes then update _exchangePricesAndConfig related data in storage always
                // update exchange prices timely before applying changes that affect utilization, rate etc.
                (, borrowExchangePrice_) = _updateExchangePrices(userBorrowConfigs_[i].token);
...
abstract contract FluidLiquidityAdminModule is AdminModuleConstants, GuardianModule {
