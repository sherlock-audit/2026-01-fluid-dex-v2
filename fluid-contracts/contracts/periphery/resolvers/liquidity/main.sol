...
import { ResolverHelpers } from "../common/helpers.sol";
...
contract FluidLiquidityResolver is IFluidLiquidityResolver, Variables, Structs, ResolverHelpers {
    ...
    function getRevenue(address token_) public view returns (uint256 revenueAmount_) {
        ...
        liquidityTokenBalance_ += _getLiquidityExternalBalances(token_, address(LIQUIDITY));
        ...
        returns (Structs.UserSupplyData memory userSupplyData_, Structs.OverallTokenData memory overallTokenData_)
    {
        ...
        if (userSupply_ > 0) {
            ...
            balanceOf_ += _getLiquidityExternalBalances(token_, address(LIQUIDITY));

            userSupplyData_.withdrawable = balanceOf_ > userSupplyData_.withdrawableUntilLimit
                ? userSupplyData_.withdrawableUntilLimit
                : balanceOf_;

            // get current leftover decay amount and original decay duration
            userSupplyData_.decayAmount = (userSupply_ >> LiquiditySlotsLink.BITS_USER_SUPPLY_DECAY_AMOUNT) & X26;
            userSupplyData_.decayAmount =
                (userSupplyData_.decayAmount >> DEFAULT_EXPONENT_SIZE) <<
                (userSupplyData_.decayAmount & DEFAULT_EXPONENT_MASK);

            uint256 decayDuration_ = (((userSupply_ >> LiquiditySlotsLink.BITS_USER_SUPPLY_DECAY_DURATION_CHECKPOINTS) &
                X10) * DECAY_CHECKPOINT_DURATION_SCALEDX10) / 10;

            if (userSupplyData_.decayAmount > 0) {
                unchecked {
                    uint256 decayed_ = block.timestamp -
                        ((userSupply_ >> LiquiditySlotsLink.BITS_USER_SUPPLY_LAST_UPDATE_TIMESTAMP) & X33);
                    // decayed_ can not underflow as last timestamp can never be > block.timestamp
                    if (decayed_ < decayDuration_) {
                        userSupplyData_.decayAmount =
                            userSupplyData_.decayAmount -
                            (userSupplyData_.decayAmount * decayed_) /
                            decayDuration_; // decayDuration_ can not be 0. can not underflow
                    } else {
                        userSupplyData_.decayAmount = 0;
                    }
                }
            }

            userSupplyData_.decayEndTimestamp = userSupplyData_.lastUpdateTimestamp + decayDuration_;
        }
    }
