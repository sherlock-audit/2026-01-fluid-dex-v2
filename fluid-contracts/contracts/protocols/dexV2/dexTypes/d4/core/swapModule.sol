...
/// @title FluidDexV2D4SwapModule
/// @notice Swap module for D4 (smart debt) DEX pools
/// @dev Implements concentrated liquidity swaps with borrow-based exchange prices
contract FluidDexV2D4SwapModule is CommonImportD4Other {
    uint256 internal constant DEX_TYPE_WITH_VERSION = 40_000;

    /// @notice Initializes the D4 Swap Module
    ...
    function swapIn(SwapInParams calldata params_) external _onlyDelegateCall returns (uint256 amountOut_, uint256 protocolFeeAccrued_, uint256 lpFeeAccrued_) {
        unchecked {
...

            {
...

                {
                    ...
                    if (params_.swap0To1) {
                        ...
                        amountInRawAdjusted_ = (params_.amountIn * LC.EXCHANGE_PRICES_PRECISION * c_.token0NumeratorPrecision) / (c_.token0BorrowExchangePrice * c_.token0DenominatorPrecision);
                    } else {
                        // NOTE: This calculation is inside unchecked but it wont overflow because params_.amountIn is limited to X128
                        amountInRawAdjusted_ = (params_.amountIn * LC.EXCHANGE_PRICES_PRECISION * c_.token1NumeratorPrecision) / (c_.token1BorrowExchangePrice * c_.token1DenominatorPrecision);
                    }
                    ...
                    (amountOutRawAdjusted_, protocolFeeAccruedRawAdjusted_, lpFeeAccruedRawAdjusted_) = _swapIn(SwapInInternalParams({
                        ...
                        token0ExchangePrice: c_.token0BorrowExchangePrice,
                        token1ExchangePrice: c_.token1BorrowExchangePrice,
                        dexType: dexType_,
                        dexId: dexId_
                    }));
                }
            ...
            if (params_.swap0To1) {
                ...
                amountOut_ = ((amountOutRawAdjusted_ * c_.token1DenominatorPrecision * c_.token1BorrowExchangePrice) / (c_.token1NumeratorPrecision * LC.EXCHANGE_PRICES_PRECISION));
                ...
                // NOTE: This calculation is inside unchecked but it wont overflow because protocolFeeAccruedRawAdjusted_ is limited to X86
                protocolFeeAccrued_ = (protocolFeeAccruedRawAdjusted_ * c_.token1DenominatorPrecision * c_.token1BorrowExchangePrice) / (c_.token1NumeratorPrecision * LC.EXCHANGE_PRICES_PRECISION);
                ...
                protocolFeeAccrued_ = (protocolFeeAccruedRawAdjusted_ * c_.token0DenominatorPrecision * c_.token0BorrowExchangePrice) / (c_.token0NumeratorPrecision * LC.EXCHANGE_PRICES_PRECISION);
    ...
    function swapOut(SwapOutParams calldata params_) external _onlyDelegateCall returns (uint256 amountIn_, uint256 protocolFeeAccrued_, uint256 lpFeeAccrued_) {
        unchecked {
            ...
            uint256 lpFeeAccruedRawAdjusted_;
            {
...

                {
                    ...
                    if (params_.swap0To1) {
                        ...
                        amountOutRawAdjusted_ = (params_.amountOut * LC.EXCHANGE_PRICES_PRECISION * c_.token1NumeratorPrecision) / (c_.token1BorrowExchangePrice * c_.token1DenominatorPrecision);
                    } else {
                        // NOTE: This calculation is inside unchecked but it wont overflow because params_.amountOut is limited to X128
                        amountOutRawAdjusted_ = (params_.amountOut * LC.EXCHANGE_PRICES_PRECISION * c_.token0NumeratorPrecision) / (c_.token0BorrowExchangePrice * c_.token0DenominatorPrecision);
                    }
            ...
            if (params_.swap0To1) {
                ...
                amountIn_ = ((amountInRawAdjusted_ * c_.token0DenominatorPrecision * c_.token0BorrowExchangePrice) / (c_.token0NumeratorPrecision * LC.EXCHANGE_PRICES_PRECISION)) + 1; // Just adding 1 so protocol remains on the winning side
