...
abstract contract ConstantVariables is CommonImportD1D2Common {
    ...
    uint256 internal constant DEX_TYPE = 1;
}

abstract contract Variables is ConstantVariables {
    /// @dev @Samyak this was not present in dexV1
    // First 64 bits => 0-63 => token 0 total supply raw (56|8 big number)
    // Next 64 bits => 64-127 => token 1 total supply raw (56|8 big number)
    // Last 128 bits => 128-255 => empty for future use
    /// @dev type => pool id => total supply
    mapping(uint256 => mapping(bytes32 => uint256)) internal _totalSupplyRaw;

    // First 128 bits => 0-127 => total supply shares
    // Last 128 bits => 128-255 => max supply shares
    /// @dev type => pool id => total supply shares
    mapping(uint256 => mapping(bytes32 => uint256)) internal _totalSupplyShares;
    ...
    /// @dev type => pool id => user address => supply data
    mapping(uint256 => mapping(bytes32 => mapping(address => uint256))) internal _userSupplyData;
}
