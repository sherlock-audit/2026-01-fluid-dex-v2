...
contract FluidDexV2D1Admin is CommonImportD1Other {
    ...
    function updateUserSupplyConfigs(DexKey memory dexKey_, UserSupplyConfig[] memory userSupplyConfigs_) external _onlyDelegateCall {
        bytes32 dexId_ = keccak256(abi.encode(dexKey_));
        _check(dexId_);
        ...
        for (uint256 i; i < userSupplyConfigs_.length; ) {
            ...
            userSupplyData_ = _userSupplyData[DEX_TYPE][dexId_][userSupplyConfigs_[i].user];

            // Updating user data on storage
            _userSupplyData[DEX_TYPE][dexId_][userSupplyConfigs_[i].user] =
                ...
                (userSupplyConfigs_[i].baseWithdrawalLimit.toBigNumber(SMALL_COEFFICIENT_SIZE, DEFAULT_EXPONENT_SIZE, BigMathMinified.ROUND_DOWN) <<
        ...
        emit LogUpdateUserSupplyConfigs(DEX_TYPE, dexId_, userSupplyConfigs_);
    }
    ...
    function updateUserWithdrawalLimit(DexKey memory dexKey_, address user_, uint256 newLimit_) external _onlyDelegateCall {
        bytes32 dexId_ = keccak256(abi.encode(dexKey_));
        _check(dexId_);
        ...
        // get current user config data from storage
        uint256 userSupplyData_ = _userSupplyData[DEX_TYPE][dexId_][user_];
        ...
        // Update on storage
        _userSupplyData[DEX_TYPE][dexId_][user_] =
        ...
        emit LogUpdateUserWithdrawalLimit(DEX_TYPE, dexId_, user_, newLimit_);
    }

    function pauseUserSupply(DexKey memory dexKey_, address user_) public _onlyDelegateCall {
        bytes32 dexId_ = keccak256(abi.encode(dexKey_));
        _check(dexId_);
        ...
        uint256 userData_ = _userSupplyData[DEX_TYPE][dexId_][user_];
        if (userData_ == 0) {
            revert(); // FluidDexError(ErrorTypes.DexT1Admin__UserNotDefined);
        }
        if (userData_ & 1 == 0) {
        ...
        _userSupplyData[DEX_TYPE][dexId_][user_] = userData_ & (~uint(1));

        emit LogPauseUserSupply(DEX_TYPE, dexId_, user_);
    }

    function unpauseUserSupply(DexKey memory dexKey_, address user_) public _onlyDelegateCall {
        bytes32 dexId_ = keccak256(abi.encode(dexKey_));
        _check(dexId_);
        ...
        _userSupplyData[DEX_TYPE][dexId_][user_] = userData_ | 1;

        emit LogUnpauseUserSupply(DEX_TYPE, dexId_, user_);
    }

    function _lockInitialAmount(
        DexKey memory dexKey_,
        bytes32 dexId_,
        uint token0Amt_,
        uint centerPrice_,
        ...
        if (msg.value > 0 && dexKey_.token0 != NATIVE_TOKEN && dexKey_.token1 != NATIVE_TOKEN) {
            revert(); // FluidDexError(ErrorTypes.DexT1Admin__InvalidParams);
        }

        for (uint i = 0; i < 2; i++) {
            if (i == 0) {
                v_.token = dexKey_.token0;
                v_.amt = token0Amt_;
            } else {
                v_.token = dexKey_.token1;
                v_.amt = v_.token1Amt;
            }
            if (v_.token == NATIVE_TOKEN) {
                if (msg.value > v_.amt) {
                    SafeTransfer.safeTransferNative(msg.sender, msg.value - v_.amt);
                } else if (msg.value < v_.amt) {
                    revert(); // FluidDexError(ErrorTypes.DexT1Admin__InvalidParams);
                }
                // deposit
                LIQUIDITY.operate{ value: v_.amt }(v_.token, int(v_.amt), 0, address(0), address(0), abi.encode(v_.amt, false, msg.sender));
            } else {
                // deposit
                LIQUIDITY.operate(v_.token, int(v_.amt), 0, address(0), address(0), abi.encode(v_.amt, false, msg.sender));
            }
        }
        ...
        v_.totalSupplyShares = (v_.token0AmtAdjusted > v_.token1AmtAdjusted)
            ? v_.token0AmtAdjusted * 10 ** (18 - TOKENS_DECIMALS_PRECISION)
            : v_.token1AmtAdjusted * 10 ** (18 - TOKENS_DECIMALS_PRECISION);

        if (v_.totalSupplyShares < NINE_DECIMALS) {
            revert(); // FluidDexError(ErrorTypes.DexT1Admin__UnexpectedPoolState);
        }

        // setting initial max shares as X128
        v_.totalSupplyShares = (v_.totalSupplyShares & X128) | (X128 << 128);
        // storing in storage
        _totalSupplyShares[DEX_TYPE][dexId_] = v_.totalSupplyShares;
    }
    ...
    function initialize(InitializeParams memory i_) public payable _onlyDelegateCall {
        ...
        _lockInitialAmount(i_.dexKey, v_.dexId, i_.token0ColAmt, i_.centerPrice, v_.token0Decimals, v_.token1Decimals);
        ...
        emit LogInitializePoolConfig(DEX_TYPE, v_.dexId, i_.token0ColAmt, i_.fee, i_.revenueCut * FOUR_DECIMALS, i_.centerPriceAddress, i_.hookAddress);
    ...
    function updateMaxSupplyShares(DexKey memory dexKey_, uint maxSupplyShares_) external _onlyDelegateCall {
        bytes32 dexId_ = keccak256(abi.encode(dexKey_));
        _check(dexId_);

        uint totalSupplyShares_ = _totalSupplyShares[DEX_TYPE][dexId_];

        // totalSupplyShares_ can only be 0 when smart col pool is not initialized
        if ((maxSupplyShares_ > X128) || (totalSupplyShares_ == 0)) {
            revert(); // FluidDexError(ErrorTypes.DexT1Admin__ConfigOverflow);
        }
        _totalSupplyShares[DEX_TYPE][dexId_] = (totalSupplyShares_ & X128) | (maxSupplyShares_ << 128);

        emit LogUpdateMaxSupplyShares(DEX_TYPE, dexId_, maxSupplyShares_);
    }
}
