...
/// @title FluidDexV2D3ControllerModule
/// @notice Controller module for D3 (smart collateral) DEX pools
/// @dev Allows pool controllers to configure dynamic fees and fee parameters
contract FluidDexV2D3ControllerModule is CommonImportD3Other {
    uint256 internal constant DEX_TYPE_WITH_VERSION = 30_000;

    /// @notice Initializes the D3 Controller Module
    ...
    /// @param dexKey_ The DexKey identifying the pool (must be called by dexKey_.controller)
    /// @param lpFee_ The new LP fee (in basis points)
    ...
    /// @param minFee_ Minimum fee (in basis points)
    /// @param maxFee_ Maximum fee (in basis points)
